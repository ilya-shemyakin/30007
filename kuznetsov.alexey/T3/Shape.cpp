...
double kuzn::Point::getDistance(const Point& point) const
{
    return std::hypot(x - point.x, y - point.y);
}

bool kuzn::Point::operator != (const Point& other) const
{
    return x != other.x || y != other.y;
}

double kuzn::getTriangleArea(const Point& pointFirst, const Point& pointSecond, const Point& pointThird)
{
    double a = pointFirst.getDistance(pointSecond),
        b = pointSecond.getDistance(pointThird),
        c = pointFirst.getDistance(pointThird);
    double perimeter = (a + b + c) / 2;
    return sqrt(perimeter * (perimeter - a) * (perimeter - b) * (perimeter - c)); //ô.Ãåðîíà
}

bool kuzn::Segment::isIntersect(const Segment& other, std::pair<double, double>* intr = nullptr) const
{
    double
        a11 = end.x - begin.x,
...
bool kuzn::Polygon::contains(const Point& pnt) const
{
...
std::vector<kuzn::Segment> kuzn::Polygon::createSegmentPool(void) const
{
...
bool kuzn::Polygon::isIntersect(const Polygon& other) const
{
...
double kuzn::Polygon::getArea() const
{
...
std::istream& kuzn::operator >> (std::istream& in, kuzn::DelimeterIO&& dest)
{
...
std::istream& kuzn::operator >> (std::istream& in, kuzn::Point& point)
{
    std::istream::sentry sentry(in);
    if (!sentry)
    {
    ...
    in >> kuzn::DelimeterIO{ '(' } >> point.x >> kuzn::DelimeterIO{ ';' } >> point.y >> kuzn::DelimeterIO{ ')' };
    return in;
}

std::istream& kuzn::operator >> (std::istream& in, kuzn::Polygon& poly)
{
    std::istream::sentry sentry(in);
    if (!sentry)
    {
    ...
    for (kuzn::Point& point : poly.points)
    {
        in >> point;
    }
    return in;
...
std::ostream& kuzn::operator << (std::ostream& out, const kuzn::Point& point)
{
...
std::ostream& kuzn::operator << (std::ostream& out, const kuzn::Polygon& poly)
{
    std::ostream::sentry sentry(out);
    if (!sentry)
    {
    ...
    for (const kuzn::Point& point : poly.points)
    {
        out << point << " ";
    }
    return out;
